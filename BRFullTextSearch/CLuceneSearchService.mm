...
- (void)bulkUpdateIndex:(BRSearchServiceIndexUpdateBlock)updateBlock queue:(dispatch_queue_t)finishedQueue finished:(BRSearchServiceUpdateCallbackBlock)finishedBlock {
    _currentQueuedTask++;
    NSLog(@"queuedTaskCount: %@", @(_currentQueuedTask));
    ...
    dispatch_async(IndexWriteQueue, ^{
        ...
        _currentQueuedTask--;
        NSLog(@"finished one Task And there Left queuedTaskCount: %@", @(_currentQueuedTask));
        ...
        switch ( [comparison predicateOperatorType] ) {
            ...
            case NSMatchesPredicateOperatorType:
            {
                ...
                try {
                    ...
                    Query *q = parser.parse(QueryParser::escape([rhs constantValueCLuceneString]), [[lhs keyPath] asCLuceneString], theAnalyzer);
                    result.reset(q);
                } catch ( CLuceneError &ex ) {
